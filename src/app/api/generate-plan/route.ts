...
You will receive the user's goal, experience level, training days per week, equipment, and free-form notes.
...
- "reps" is a string so you can use ranges like "8-10" or "AMRAP" or "30s" for cardio/holds.
- "notes" should be short or empty string — RPE, tempo, or progression tips.
- Keep "focus" to 1-3 words like "Push", "Pull", "Legs + core", "Conditioning".
- Respect the user's constraints (injuries, preferences) from the notes.`;

type Exercise = { name: string; sets: number | null; reps: string | null; notes: string | null };
type PlanDayPayload = {
  day_of_week: number;
  focus: string | null;
  is_rest_day: boolean;
...
function parsePlan(text: string): PlanDayPayload[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const exs: Exercise[] = isRest
      ? []
      : exsIn.map((e: { name?: unknown; sets?: unknown; reps?: unknown; notes?: unknown }) => ({
...
export async function POST(request: Request) {
  ...
  try {
    const userMsg =
      ...
      (notes ? `Notes: ${notes}\n` : "") +
      `\nReturn the 7-day JSON plan now.`;
